...
- **算法4.1 BF 算法 Brute-Force**
  ...
  - ［算法分析］
    ...
    (1)最好情况下，每趟不成功的匹配都发生在`模式串的第一个字符`与主串中相应字符的比较。

    设主串的长度为n, 子串的长度为m, 假设从主串的第`i`个位置开始与模式串匹配成功，则在前`i-1` 趟匹配中字符总共比较了`i-1` 次；若第`i `趟成功的字符比较次数为m, 则总比较次数为`i-1+m`。对于成功匹配的主串, 其起始位置由1到`n-m+1`, 假定这`n-m+1`个起始位置上的匹配成功概率相等, 则最好的情况下匹配成功的平均比较次数为`(n+m)/2`.

    最好情况下的平均时间复杂度是O(n + m)。

    (2 ,)最坏情况下， 每趟不成功的匹配都发生在`模式串的最后一个字符`与主串中相应字符的比较。
